...
document.addEventListener("DOMContentLoaded", function() {
 ...
 document.addEventListener("DOMContentLoaded", function(){
  ...
  subButton.addEventListener("click", function(x){
    ...
    if(isValid){
      if(totalNum < 20 || totalNum > 50){
        error.textContent = "The number of attendees does not meet the minimum requirements (20)"
        error.style.color = "red"
        error.style.display = "block"
        isValid = false
      }
    ...
    if(isValid){
      ...
      if(hours > 2){
        error.textContent = "Maximum duration for Multifunctional Area booking is 2 hours"
        error.style.color = "red"
        error.style.display = "block"
        isValid = false
      }
